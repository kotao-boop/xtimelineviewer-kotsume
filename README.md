...
> [!WARNING]
> Microsoft Store版はMicrosoftが署名して配布します。GitHubで配布するv2.5.1のEXE・ZIPはコード署名されていないため、Windowsで「不明な発行元」やSmartScreenの警告が表示される場合があります。実行前に、リリースに添付された`SHA256SUMS.txt`とGitHub Artifact Attestationでファイルを確認してください。これらは改ざん確認の助けになりますが、コード署名や安全性の保証そのものではありません。

| 形式 | ファイル名 | 説明 |
|---|---|---|
| **インストーラー版 (EXE)** | `XTimelineViewer-Kotsume-v2.5.1-Setup.exe` | ダブルクリックで実行する標準インストーラー。デスクトップアイコン作成、スタートメニュー登録、アンインストールに対応。 |
| **ポータブル版 (ZIP)** | `XTimelineViewer-Kotsume-v2.5.1-win-x64-Portable.zip` / `...-win-arm64-Portable.zip` | 解凍して `XTimelineViewer.exe` を起動するだけで使える自己完結パッケージ。CPUに合う版を選びます。 |

### Microsoft Store版の状況

Microsoft Store版は2026年8月27日に初回公開されました。次回のStore提出では、v2.5.0までの改善に加えて、自動翻訳ボタンを列ヘッダーの三点メニューへまとめ、表示場所を選べるv2.5.1相当の内容を提出する予定です。Google／Appleログインがアプリ内で完了できない場合は、X用パスワードによる確実なログイン方法を画面内で案内します。
...
**Status:** Microsoft Store distribution is live and Store packages are signed by Microsoft. The GitHub v2.5.1 EXE/ZIP release remains unsigned while the separate SignPath application is deferred.
