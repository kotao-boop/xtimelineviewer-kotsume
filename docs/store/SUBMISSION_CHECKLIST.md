...
> **現在の状態（2026年9月10日）:** v2.8.6.0のx64版・ARM64版をSubmission 13として提出し、
> Partner Centerでは「In certification」です。日本語・英語のStore掲載情報はv2.8.6用へ更新済みです。
> 認定と公開が完了するまでは、Storeで公開中の版はv2.8.5.0です。初回合格を次回更新の合格保証として扱いません。
...
- [ ] 「同意設定」からGoogle方式を無効化するとGoogleへの翻訳通信が発生しないことを確認した
- [ ] Google方式を再び有効にした後、投稿本文の送信に再同意が必要になることを確認した
...
- [ ] MSIX版でも、設定画面から利用者追加拡張を追加・再起動後に読み込めることを確認した
- [ ] MSIX版の同梱拡張更新が利用者追加拡張を削除しないことを確認した
...
- [ ] v2.8.6用の個人情報が写っていないスクリーンショットを4枚以上保存した
- [x] v2.8.6用の日本語・英語の説明、機能、「新機能」、検索語案を保存した
- [x] カテゴリと年齢区分
- [x] プライバシーポリシーURL
- [x] サポートURL
